...
class Core50(Dataset):
    ...
    def __init__(self, root: str, train: bool=True, transform: transforms=None,
        ...
        self.root = os.path.join(base_data_path(), 'CORE50')
        self.train = train
        ...
        if download:
            if os.path.isdir(self.root) and len(os.listdir(self.root)) > 0:
                ...
                gdd.download_file_from_google_drive(
                    ...
                    dest_path=os.path.join(base_data_path(), 'core-50-processed.zip'),
                    unzip=True)

        self.targets = []
        for num in range(24 if self.train else 9):
            self.targets.append(np.load(os.path.join(
                base_data_path(), 'CORE50/core50/CORE50/processed/y_%s_%02d.npy' %
            ...
            if not memory_bank_index in self.in_memory:
                ...
                if self.train:
                    ...
                    if len(self.TRAIN_MAP[memory_bank_index]) == 1:
                        ...
                        for b in task_banks:
                            self.loaded_data[b] = np.load(os.path.join(base_data_path(), 'CORE50/core50/CORE50/processed/x_train_%02d.npy' % b))
                            self.in_memory.append(b)
                    else:
                        self.loaded_data[memory_bank_index] = np.load(os.path.join(base_data_path(), 'CORE50/core50/CORE50/processed/x_train_%02d.npy' % memory_bank_index))
                    ...
                    self.loaded_data[memory_bank_index] = np.load(os.path.join(base_data_path(), 'CORE50/core50/CORE50/processed/x_test_%02d.npy' % memory_bank_index))
                    self.in_memory.append(memory_bank_index)
...
class SequentialCore50(ContinualDataset):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = MyCore50(base_data_path() + 'CORE50/core50/CORE50',
                                 train=True, download=True, transform=transform)
        if self.args.validation:
            train_dataset, test_dataset = get_train_val(train_dataset,
                                                    test_transform, self.NAME)
        else:
            test_dataset = Core50(base_data_path() + 'CORE50/core50/CORE50',
    ...
    def not_aug_dataloader(self, batch_size):
       ...
       train_dataset = MyCore50(base_data_path() + 'CORE50/core50/CORE50', train=True,
    ...
    def get_backbone(attention='ae_sigmoid', code_dim=64):
        return resnet18(np.sum(SequentialCore50.N_CLASSES_PER_TASK), n_tasks=SequentialCore50.N_TASKS, attention=attention,
                        code_dim=code_dim)
